...
def initiate_payment(order, user_email):
    ...
    if existing_payment:
        if existing_payment.status == 'success':
            ...
            cache.delete(f'payment_{existing_payment.reference}')
...
def verify_payment(reference):
    # check cache first
    cache_key = f'payment_{reference}'
    ...
    if not payment:
        try:
            payment = (
                Payment.objects
                .select_related('order', 'order__user', 'order__product')
                .get(reference=reference)
            )
        except Payment.DoesNotExist:
            return None, 'Payment not found.'

    # if payment already verified, return cached result
    ...
    # cache the verified payment for 10 minutes
    cache.set(cache_key, payment, timeout=60 * 10)
...
def handle_webhook_event(event, data):
    ...
    try:
        payment = (
            Payment.objects
            .select_related('order')  # ← fetches order in ONE query
            .get(reference=reference)
        )
    except Payment.DoesNotExist:
        return False
